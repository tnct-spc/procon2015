...
public:
    ...
    std::size_t NOW_FIRST_STONE_NUM;

private:
    static constexpr std::size_t MAX_SEARCH_DEPTH = 15;
    static constexpr std::size_t MAX_SEARCH_WIDTH = 3;
    static constexpr std::size_t HOLD_FIELD_NUM = 7;
    std::size_t ALL_STONES_NUM;

    class one_try
    {
    public:
        one_try(problem_type _problem, evaluator _eval, sticky_beam* _parent);
        ~one_try();
        void one_try_run(int y, int x, std::size_t angle, int side);
